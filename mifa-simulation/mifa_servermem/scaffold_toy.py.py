...
class Server():
    ...
    def aggregate(self, ids, client_models, algo, scaff_mods, delta_c_i, w_opt):
        ...
        wnewx = global_state_dict['n1.weight']
        
        self.global_model.load_state_dict(global_state_dict)    

        return np.linalg.norm(wnewx-w_opt)**2       
...
class Client():
    ...
    def sgd_step(self, dtrain_loader, model):

      v1 = model.state_dict()['n1.weight'][0].numpy()
      ...
      for t in range(tau):
        ...
        A_mat_i, b_mat_i = next(iter(dtrain_loader))
        A_mat_i, b_mat_i = A_mat_i.numpy(), b_mat_i.numpy().reshape((2,1))

        grad = A_mat_i[0].dot(v1) - b_mat_i

    
        v1 = v1-learning_rate*grad

      new_mod.load_state_dict({'n1.weight':torch.tensor(v1.reshape((1,2)))})
      return new_mod, None

  
    def scaffold_step(self, dtrain_loader, model, cnt,c):
        ...
        if 'n1.weight' not in self.c_i:
          self.c_i.setdefault('n1.weight',torch.tensor([0.0]))
          c_i = np.array([0])
        else:
          c_i = self.c_i['n1.weight'][0].numpy()
          c_i = c_i.reshape((2,1))

        c = c['n1.weight'][0].numpy()
        c = c.reshape((2,1))
        ...
        for t in range(tau):
          ...
          grad_corr = grad - c_i + c
          
          v1 = v1-learning_rate*grad_corr
    ...
    def local_train(self,dtrain_loader,lr, model, algo = None, c = None, w_opt = 0):
        ...
        #     loss+=loss_epoch
        if algo == 'scaffold':
          model_trained, cnt = self.scaffold_step(dtrain_loader, model, cnt, c=c)
        else:
          model_trained, cnt = self.sgd_step(dtrain_loader, model)
        newx = model_trained.cpu().state_dict()
        #print("loss",loss)
        epoch +=1
        wnewx = newx['n1.weight'][0]
        ...
        if algo == 'scaffold':
            ...
            for layer, val in newx.items(): #torch.Tensor([0.0])#
                new_c_i_layer = self.c_i[layer] - c[layer] + ((oldx[layer]-local_grad_update[layer]).float()/(cnt*lr))
            ...
            return local_grad_update, delta_c_i, loss/config.local_epochs, np.linalg.norm(wnewx-w_opt)**2
            ...
            with torch.no_grad():
                ...
                for layer, val in newx.items():
                    ...
                    local_grad_update[layer] = (newx[layer]-oldx[layer]).float()

        return local_grad_update, loss/config.local_epochs, np.linalg.norm(wnewx-w_opt)**2
...
if __name__ == "__main__":
    ...
    if config.dataset == 'toy':
      d =2
      dataset_train = []
      b_mat = np.zeros((config.total_c,d))
      A_mat = np.zeros((config.total_c,d,d))
      b_mat_sum = np.zeros((d))
      A_mat_sum = np.zeros((d,d))
      A_mat_all = np.array([[1,1],[1,2]])
      dict_users = {}
      cnt = 0
      for i in range(config.total_c):
            dict_users.setdefault(i,[])
            b_mat[i] = np.random.normal(2,2,(d))
            A_mat[i] = A_mat_all + np.random.normal(0,0.1,(d,d)) ###creating heterogeneity among clients
            dataset_train.append([A_mat[i], np.array(b_mat[i])])
            #dataset_train.append([A_mat[i][1], np.array([b_mat[i][1]])])
            dict_users[i].extend([cnt])
            cnt+=1
            b_mat_sum += b_mat[i]
            A_mat_sum += A_mat[i]
      w_opt = np.linalg.inv(A_mat_sum).dot(b_mat_sum)
    ...
    elif config.model_type == 'toy':
        cmodel = network.Toy()
        init_w = {'n1.weight':torch.tensor([[-1,-1]])}
        cmodel.load_state_dict(init_w)
    ...
    for choose_nc in config.no_of_c:
        ...
        for learning_rate in range(len(config.algo_lr[0])):
            ...
            for algo in list(d_algo.keys()): 
                ...
                for rnd in (range(config.n_rnds)): # each communication round
                    ...
                    for sel_idx in idxs_users:
                        ...
                        if algo==3:
                            local_grad_up, delta_c_i, loss, mse = client_object_dict[sel_idx].local_train(dtrain_loader, lr = lr,  model = copy.deepcopy(server.global_model), algo = d_algo[algo], c = copy.deepcopy(server.c_i), w_opt =w_opt)
                            scaff_mods.append(local_grad_up)
                            delta_c_i_l.append(delta_c_i)
                        else:
                            local_grad_up, loss, mse = client_object_dict[sel_idx].local_train(dtrain_loader, lr = lr,  model = copy.deepcopy(server.global_model), algo = d_algo[algo], c = copy.deepcopy(server.c_i), w_opt = w_opt)
                            scaff_mods.append(local_grad_up)
                            # print(loss)
                    ...
                    #aggregate at server
                    if algo==3:
                        wmse = server.aggregate(idxs_users, client_object_dict, algo, scaff_mods, delta_c_i_l, w_opt = w_opt)
                    else:
                        wmse = server.aggregate(idxs_users, client_object_dict, algo, scaff_mods , None, w_opt = w_opt)
                    local_rnd_loss.append(wmse)
                    #Uncomment 
                    ...
                    print("Rnd {4} - Training Error: {5}, mse:{0}, Algo: {2}, n_c: {3}, lr: {1}\n".format(wmse,lr, algo, choose_nc, rnd, d_train_losses))
                    # #mem leak
            ...
            start_pos = 0
            for i, key in enumerate(loss_algo):
                plt.plot(key,label=str(i))

                plt.xlabel('Round No.')

                plt.ylabel("Distance to optima")
                plt.yscale('log')
                plt.xscale('log')
                plt.legend()
                plt.savefig('mseloss')
